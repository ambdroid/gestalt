    ...
    try:
        pairs = dict(
            reduce(
                lambda cur, nxt: (
                    ParseState(
                        ChainMap({cur.stack[0]: cur.pos}, cur.pairs),
                        cur.stack[1],
                        cur.pos + 1,
                    )
                    if nxt == ")"
...
types = {
    ...
    "if": lambda args: (len(args) == 3 and args[0] == bool and args[1] == args[2])
    and args[1],
    "one": typecheck(int),
...
def comp(ast, index=0):
    if type(ast) in [int, str, bool]:
        ...
        post = [ast.op]
    return (
        reduce(lambda cur, nxt: cur + comp(nxt, index + len(cur)), ast.args, []) + post
    )
    ...
    class Inner:
        ...
        def to_json(self):
            return json.dumps(
                self.to_dict(),
                default=lambda val: (
                    val.to_dict() if dc.is_dataclass(val) else list(val)
                ),
            )
...
class ActionJoin(VotableAction, _type=ActionType.join):
    ...
    def execute(self, bot, autoadd=False):
        nick = bot.fetchone("select nick from masks where maskid = ?", (self.mask,))
        if nick:
...
class ActionServer(VotableAction, _type=ActionType.server):
    ...
    def execute(self, bot):
        if bot.fetchone("select 1 from masks where maskid = ?", (self.mask,)):
            if bot.get_guild(self.server) and not bot.is_mask_in(
                self.mask, self.server
            ):
                bot.execute(
                    "insert into guildmasks values"
                    "(?, ?, NULL, NULL, NULL, ?, ?, NULL)",
...
class ActionChange(VotableAction, _type=ActionType.change):
    ...
    def valid(word):
        if (
            ret := {"name": "nick", "colour": "color"}.get(word, word)
    ...
    async def execute(self, bot):
        if self.which == "avatar":
            if not (
                prev := bot.fetchone(
                    "select avatar from masks where maskid = ?", (self.mask,)
                )
            ):
                return
            prev = prev[0]
...
class ActionRules(VotableAction, _type=ActionType.rules):
    ...
    @classmethod
    def class_dict(cls, _dict):
        return super().class_dict(
            _dict | {"newrules": Rules.from_dict(_dict["newrules"])}
        )
...
class RulesLegacy(Rules, _type=RuleType.legacy):
    ...
    async def eval(self, bot, context, action):
        ...
        if not member or type(action) == ActionServer:
            return
        elif type(action) == ActionChange or member.guild_permissions.manage_roles:
        ...
        raise UserError(
            ...
            + (
                ("(The <@&%i> role itself is no longer necessary.)" % self.role)
                if self.role != self.guild
                else ""
            )
        )
...
class RulesUnanimous(Rules, _type=RuleType.unanimous):
    ...
    def for_action(self, atype):
        return Exp(
            "or",
            (rule_solo, self.rule_remove if atype == ActionType.remove else self.rule),
        )
...
class Vote(metaclass=serializable):
    ...
    async def after_interaction(self, interaction, desc, bot):
        ...
        embed = (
            embed if interaction.message.embeds[0] != embed else discord.utils.MISSING
        )
    ...
    def inactive(self):
        return self.complete or (
            discord.utils.utcnow() - discord.utils.snowflake_time(self.context.message)
...
class VoteCreate(VoteConfirm, _type=VoteType.create):
    ...
    def description(self):
        if self.complete:
            return "Mask **%s** created!" % discord.utils.escape_markdown(self.name)
        else:
            return (
                "Do you want to automatically try to add this mask to all "
...
class VotePreinvite(VoteConfirm, _type=VoteType.preinvite):
    ...
    async def on_done(self, bot):
        if self.yes:
            await bot.initiate_action(
                self.context, ActionInvite(self.mask, self.get_user())
            )
...
async def respond(interaction, content):
    await interaction.response.send_message(
        embed=discord.Embed(description=content), ephemeral=True
    )
...
class GestaltVoting:
    def load(self):
        self.votes = {
            row["msgid"]: Vote.from_json(row["state"])
    ...
    def get_rules(self, maskid):
        if row := self.fetchone("select rules from masks where maskid = ?", (maskid,)):
    ...
    async def initiate_action(self, context, action):
        ...
        context.members = frozenset(
            ...
            for row in self.fetchall(
                "select userid from proxies where maskid = ?", (action.mask,)
            )
        )
    ...
    async def initiate_vote(self, vote):
        ...
        if not channel.guild:
            ...
            recipient = (
                channel.recipient or (await self.fetch_channel(chanid)).recipient
            )
            if vote.eligible != frozenset({recipient.id}):
                return await self.send(
                    channel, "A vote was called for, but it must be run in a guild."
                )
        if msg := await self.send(
            channel,
            vote.description(),
            view=vote.view(),
    ...
    async def try_auto_add(self, userid, guildid, maskid):
        if not self.is_mask_in(maskid, guildid):
            ...
            # shrug.
            await self.initiate_action(
                ProgramContext(initiator=userid), ActionServer(maskid, guildid)
            )
    ...
    async def on_bot_interaction(self, message):
        ...
        if not (
            votes := [
                msgid
                for (msgid, vote) in self.votes.items()
