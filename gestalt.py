...
class Gestalt(discord.Client, commands.GestaltCommands, gesp.GestaltVoting):
    ...
    def fetchone(self, *args):
        return self.cur.execute(*args).fetchone()
    ...
    async def update_status(self):
        ...
        await self.change_presence(
            ...
            activity=discord.Game(
                name="%shelp%s" % (COMMAND_PREFIX, (motd and " | %s" % motd))
            ),
        )
    ...
    async def try_add_reaction(self, message, reaction):
        if self.has_perm(
            message.channel, add_reactions=True, read_message_history=True
        ):
            try:
    ...
    async def mark_success(self, message, success):
        await self.try_add_reaction(message, REACT_CONFIRM if success else REACT_DELETE)
    ...
    async def send(
        self, channel, content="", plain="", embeds=[], view=None, reference=None
    ):
        if self.has_perm(channel, send_messages=True):
            try:
                return await channel.send(
                    plain,
                    embeds=(
                        ([discord.Embed(description=content)] if content else [])
                        + embeds
                    ),
                    view=view,
                    reference=reference,
                ...
                if reference and e.code == 50035:
                    ...
                    return await self.send(channel, content, plain, embeds, view)
                raise
    ...
    def get_tags_conflict(self, userid, pair):
        ...
        return [
            ...
            and (
                (
                    prefix.startswith(proxy["prefix"])
                    and postfix.endswith(proxy["postfix"])
                )
                or (
                    proxy["prefix"].startswith(prefix)
    ...
    def get_proxy_swap(self, message, proxy):
        ...
        if member:
            color = self.fetchone(
                "select color from users where userid = ?", (member.id,)
            )["color"]
            return {
                "username": member.display_name,
                "avatar_url": member.display_avatar.replace(format="webp"),
                "color": color,
            }
    ...
    def get_proxy_mask(self, message, proxy):
        ...
        if mask := self.fetchone(
            "select nick, avatar, color from masks where maskid = ?", (proxy["maskid"],)
        ):
            return {
                "username": mask["nick"],
    ...
    async def get_webhook(self, channel, create=False):
        ...
        if row := self.fetchone(
            "select * from webhooks where chanid = ?", (channel.id,)
        ):
            return discord.Webhook.partial(row[1], row[2], session=self.session)
        if create:
            try:
    ...
    async def make_log_message(self, message, orig, proxy=None, old=None):
        ...
        logchan = self.fetchone(
            "select logchan from guilds where guildid = ?", ((old or orig).guild.id,)
        )
        if not logchan:
            return
        ...
        if proxy:
            footer = (
                (
                    "Mask ID: %s | " % proxy["maskid"]
                    if proxy["type"] == ProxyType.mask
                    else ""
                )
                + "Proxy ID: %s | " % proxy["proxid"]
            ) + footer
        embed.set_footer(text=footer)
        await self.send(
            self.get_channel(logchan[0]),
            plain=
    ...
    async def do_proxy(self, message, content, proxy, prefs):
        ...
        if proxy["become"] is not None and proxy["become"] < 1.0:
            self.set_autoproxy(
                message.author, proxy["proxid"], become=proxy["become"] + 1 / BECOME_MAX
            )
            ...
            if reference:
                embed = discord.Embed(
                    description=(
                        ...
                        else f"*[(click to see attachment)]({reference.jump_url})*"
                    )
                )
                if present["color"]:
        ...
        thread = channel if type(channel) == discord.Thread else discord.utils.MISSING
        am = discord.AllowedMentions(
            everyone=channel.permissions_for(message.author).mention_everyone
        )
        if not (
            new := await self.execute_webhook(
                channel,
                thread=thread,
        ...
        if not proxy["flags"] & ProxyFlags.echo:
            await self.try_delete(
                message, delay=DELETE_DELAY if prefs & Prefs.delay else None
            )
    ...
    def proxy_visible_in(self, proxy, guild):
        ...
        if proxy["type"] == ProxyType.override:
            return True
        elif proxy["type"] in (ProxyType.swap, ProxyType.pkswap, ProxyType.pkreceipt):
    ...
    def get_proxy_match(self, message):
        ...
        if not (
            ...
            match = discord.utils.find(
                lambda proxy: proxy["proxid"] == member["ap"], proxies
            )
    ...
    async def on_user_message(self, message, user):
        ...
        chan = self.fetchone(
            "select * from channels where chanid = ?", (message.channel.id,)
        )
        ...
        if reader := commands.CommandReader.from_message(message):
            if mandatory:
                ...
                raise UserError("You cannot use commands in a Mandatory mode channel.")
        ...
        if (
            message.guild
            and user["prefs"] & Prefs.homestuck
            and (match := BE_REGEX.fullmatch(content))
        ):
            try:
        ...
        if mandatory and (
            not match
            or match["state"] != ProxyState.active
            or match["type"] == ProxyType.override
            or (match["type"], match["otherid"]) == (ProxyType.swap, authid)
        ):
            await self.try_delete(message)
            return
    ...
    async def on_message(self, message):
        ...
        if (
            authid == PK_ID
            and message.channel.id in self.expected_pk_errors
        ...
        if (
            message.channel.type not in ALLOWED_CHANNELS
        ...
        if (
            ...
            user = self.fetchone("select * from users where userid = ?", (authid,))
            try:
        ...
        if emoji == REACT_QUERY:
            ...
            if payload.user_id in (row["authid"], row["otherid"]):
                if not await self.try_delete(message):
                    await self.reply(message, "I can't delete messages here.")
            else:
                if self.has_perm(message.channel, manage_messages=True):
                    await message.remove_reaction(emoji, reactor)
...
def main():
    instance = Gestalt(dbfile=sys.argv[1] if len(sys.argv) > 1 else DEFAULT_DB)
